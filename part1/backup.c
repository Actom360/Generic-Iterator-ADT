...
typedef struct IteratorIntRep {
    ...
    Node *prev;
    Node *next;
    int setdelcall; // goes from 0 to 1 when prev/next/hasprev/hasnext 
    Node *setdel;
} IteratorIntRep;
...
int delete(IteratorInt it){
    if (it->setdelcall == CANNOT_SETDEL || it->nitems == 0 || it->setdel == NULL) 
    ...
    { 
        if (it->setdelcall == CANNOT_SETDEL) {
    ...
    { 
        ...
        free(it->setdel);
        it->setdel = NULL;
        it->setdelcall = CANNOT_SETDEL; // make it so you can't delete again until prev/next/findprev/findnect is called
        return 1;
